...
class HarryPotterDataset(Dataset):
    def __init__(
        self,
        data_dir="./data",
        data_file="harrypotter_all_books.txt",
        block_size=8,
        download=True,
    ):
        super().__init__()
        ...
        if (not Path(f"{self.data_dir}/{self.data_file}").exists()) and download:
            urls = [
            ...
            for url in tqdm(urls):
                ...
                with open(Path(self.data_dir) / self.data_file, "a") as f:
                    f.write(res.text)
        else:
            print("Data files already downloaded!")

        with (Path(self.data_dir) / self.data_file).open() as f:
            self.text = f.read()
...
class HarryPotterDataModule(LightningDataModule):
    ...
    def prepare_data(self):
        HarryPotterDataset(
            self.hparams.data_dir, download=True, block_size=self.hparams.block_size
        )
